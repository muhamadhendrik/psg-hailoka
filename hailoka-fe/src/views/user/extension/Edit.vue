...
const extId = route.params.extId
...
async function fetchExtension() {
  try {
    ...
    const response = await extensionServices.getExtensionByExtId(extId as string )

    form.value.name = response.data.data.name
    selectedIcon.value = response.data.data.icon

    const operational = response.data.data.operational

    days.value = days.value.map((day) => {

        const match = operational.find((op: any) => op.day_of_week === day.day_of_week);

        if (match) {

            const [startH, startM, startS] = match.start_time.split(":").map(Number);
            const [endH, endM, endS] = match.end_time.split(":").map(Number);

            return {
                ...day,
                checked: true,
                start_time: new Date(0, 0, 0, startH, startM, startS),
                end_time: new Date(0, 0, 0, endH, endM, endS),
            };
        } else {
            // keep unchecked and default values
            return {
                ...day,
                checked: false,
                start_time: new Date(0, 0, 0, 0, 0),
                end_time: new Date(0, 0, 0, 0, 0),
            };
        }
    });

    response.data.data.assignedStaffUserModel.map((data:any) => {
        staffSelection.add({
            user_id: data.id,
            name: data.name
        })
...
const handleSubmit = async () => {
  ...
  const res = await extensionServices.updateExtension(updateExtSubmit, extId as string)
  ...
  if (res.status == 201) {
    ...
    toast.add({
        severity: "info",
        summary: "Success",
        detail: 'Extension has been successdully updated',
        group: "custom",
        life: 100000,
    });
