...
async function handleLogin() {
    ...
    try {
        ...
        if (response.user?.user_type === "superadmin") {
            ...
            error.value = "Akses ditolak. Hanya super admin yang dapat login di sini.";
            // Logout user if not superadmin
...
const handleGoogleCredentialResponse = async (
    ...
    try {
        const loginResponse = await authStore.loginWithGoogle(idToken);
        
        // Check if user_type is superadmin
        if (loginResponse?.user?.user_type === "superadmin") {
...
<template>
    <MainLayout>
        ...
        <div
            class="hidden lg:block w-1/2 h-full bg-[url('/images/rectangle-orange.png')] bg-[length:1020px] bg-center rounded-s-2xl"></div>
